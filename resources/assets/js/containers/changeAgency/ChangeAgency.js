...
export default class ChangeAgency extends Component {
  ...
  componentDidMount () {
    ...
    const requestUrl = 'http://opencartecomptable.herokuapp.com/public/api' + url
    axios.get(requestUrl)
      .then(school => {
        if (school.data[0]['code_uai'] === school.data[0]['code_uai_agence_comptable']) {
          axios.get('http://opencartecomptable.herokuapp.com/public/api/agences/' + school.data[0]['code_uai'])
          .then(schools => {
  ...
  handleSubmission (agency) {
    ...
    axios.put('http://opencartecomptable.herokuapp.com/public/api/etablissements/' + codeUai, agency)
      .then(response => {
